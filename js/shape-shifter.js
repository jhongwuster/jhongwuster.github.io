...
S.UI = (function () {
  ...
  function bindEvents() {
	...
	function isScrolledIntoView(elem)
	{
		...
		return ($elem.offset().top == 50);
	}
	
	/*
	$(window).scroll(function() {
		if (isScrolledIntoView(document.querySelector('.movie'))) {
			firstAction = false;
			reset();
			performAction(simulateDefault);
		} else {
			S.Shape.shuffleIdle();
			reset(true);
		}
